...
- **静态类型**：变量在源代码中声明时所具有的类型
- **动态类型**：变量在运行时实际引用的对象的类型
- **e.g.** `#!cpp Shape *p = new Circle();` `p` 的静态类型是 `Shape`，动态类型是 `Circle`

3. **绑定** Binding：确定要调用哪个函数

- **静态绑定**：根据**声明类型**调用函数，在编译时确定
- **动态绑定**：根据对象的实际类型调用函数，在**运行时**确定
