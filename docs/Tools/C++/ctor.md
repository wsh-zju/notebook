...
1. **类的构造函数**：用于初始化对象，在创建对象时**自动调用**
    ...
    - **默认构造函数**：<mark>无参数</mark>的构造函数，默认调用
    ...
    !!! abstract "初始化列表"
        ...
        4. 初始化顺序是<mark class="blue">**按照声明顺序进行的**</mark>，而非列表中的顺序
        5. **实例中 `x` 将会赋值成乱码**，`y` 赋值成 `value`（因为先初始化的是 `x`）

2. **自动默认构造函数**：当你定义一个类且没有编写任何构造函数时，**编译器会自动**为你生成一个默认构造函数

    !!! warning "Warning"
        1. 自动生成的构造函数不负责**基本类型**（`int`, `float`, 指针等）的初始化，它们的值是<mark class="orange">**随机的**</mark>
...
3. **类的析构函数**：用于销毁对象，在销毁对象时**自动调用**
    ...
    - <mark>析构函数执行的顺序是**构造函数的逆序**</mark>

---
## 重载函数

1. **重载函数**：在同一个类中定义多个<mark>名称相同但参数列表不同</mark>的函数
2. **参数列表不同**：通过参数的数量、类型或<mark>顺序</mark>来区分不同的构造函数
...
2. **目标构造函数**：真正执行<mark>初始化列表</mark>的构造函数
3. **代理构造函数**：调用目标构造函数的构造函数
4. **优点**：可以减少重载构造函数中的**代码重复**
5. **用法**：将对另一个构造函数的调用放在<mark>**初始化列表**</mark>中

    !!! warning "Warning"
        <mark class="orange">但代理构造函数不能在其自身的初始化列表中初始化其他成员</mark>
        ...
        **解决方案**：可以使用**私有**构造函数来为其他成员提供初始化（下边第七条的例子）
...
7. 可以创建一个**代理构造函数链**
    ...
    public:
        ...
        Info(char e) : Info(1, e) {}      // 3: 委托 （使用 4）
    private:
...
2. **注意事项**：
    - 默认参数<mark>必须**从右往左**设置，不能跳过某个参数</mark> **e.g.** `#!cpp void print(int a = 1, int b)`
    - 默认参数通常<mark>**只在函数声明中指定一次**</mark>，不要在定义中重复指定
    - **默认参数与重载的冲突**：当默认参数遇到函数重载时，容易产生<mark class="orange">二义性错误</mark>
...
3. **带默认参数的构造函数**：<mark>所有参数都有默认值</mark>的构造函数就是一个**默认构造函数**
...
2. **拷贝构造函数调用的时机**
    ...
    - **函数返回**
        - <mark>返回的是对象本身，不是指针和引用</mark>
        - <mark>初始化会调用拷贝构造函数，但是赋值不会</mark>
        ...
        !!! abstract "返回对象的拷贝"
            ...
            2. **具名对象返回**：<mark class="cyan">会调用拷贝构造函数</mark>（现代编译器通**常会优化掉此拷贝**）
...
!!! tip "参数传入传出"
    **参数的传入传出方式**有三种：对象本身、指针、引用
    ...
    4. <mark class="green">如果在函数**内部创建**了一个对象，请将其作为对象传出</mark>
    5. **仅传出传入对象的指针或引用**
    6. <mark class="orange">**绝对不要在函数内 `new` 对象并返回其指针**</mark>

## Move Ctor

1. **移动构造函数**：参数为<mark>右值引用</mark>的拷贝构造函数（**直接使用原来的内存，不用再拷贝到另外一块内存**）
...
!!! example "Example"
    **关键**：在移动构造函数中，将源对象的指针置空，防止其析构时释放内存（移动构造让 `a` 和 `b` 指向同一块内存，置为零防止 `a` 的析构影响到 `b`）
