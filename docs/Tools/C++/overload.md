...
- **全局函数**：如果**左侧**操作数发生变化，则以**引用**方式传递（赋值运算符）
...
1. **前置与后置的区分**：后置接收一个 `#!cpp int` 参数，编译器将传入 `0` 作为该 `#!cpp int` 参数
...
1. **必须是全局函数**，一般设置为**友元**（可以每一个类对应一对输入输出）
2. **参数**：必须是两个参数

- **第一个参数**：`#!cpp istream&` or `#!cpp ostream&`
- **第二个参数**：一个值的**引用**
    - **输出**：使用 `#!cpp const T&`（只读取不修改）
    - **输入**：使用 `#!cpp T&`（需修改）

3. **返回值**：`#!cpp istream&` or `#!cpp ostream&`（支持链式操作）
...
2. **`#!cpp explicit`** 关键字：将一个参数的构造函数声明为**显式转换**（防止隐式转换）
...
3. **将本类型转换为另一个类型**：`X::operator T ()`

- 运算符名称可以是**任何类型**描述符
- **没有显式参数**
- **没有返回类型声明**，返回类型与函数名称 `T` 相同
