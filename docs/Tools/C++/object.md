...
??? info "扩展：constexpr"
    1. `constexpr` 是 C++11 引入的关键字，用于指示表达式或函数可以**在编译期求值**

    2. `constexpr` 可以用来**修饰函数**
    
    - 如果函数的参数是字面量或者编译时常量，则可以在编译器直接求值
    - 如果参数时运行时变量，则**不能在编译期求值**
...
3. 只要是将地址传入函数，如果有可能，**应该将其设为 `const`**
...
- 对于一个常量对象，只能调用**常量成员函数**，不能调用**非常量成员函数**
...
- 类中的 `const` 成员变量不能在**构造函数体内**赋值，而必须**通过初始化列表来初始化**
- 类中的**运行时常量**：
    - **无论是否赋值**，都是运行时常量
    - 通常不能作为成员变量数组的大小
