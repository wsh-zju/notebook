...
|:--:|:--:|:--:|:--:|:--:|:--:|
|relation|关系|reflexive|自反的|symmetric|对称的|
|antisymmetric|反对称的|closure|闭包|partition|划分|
|partial order|偏序|poset|偏序集|total/linear order|全序/线序|
|well-ordered set|良序集|lexicographic order|字典顺序|Hasse diagram|哈塞图|
|lattices|格|equivalence|等价|class|类|
|congruence|同余|maximal|极大的|minimal|极小的|
...
- **定义**
    ...
    - **划分**：集合 $S$ 的划分为其**不相交的非空子集**构成的集合，并且他们的并集为 $S$
### 定理 1
...
- **定义**
    ...
    - 偏序集 $(S,\preceq)$ 中的元素 $a$ 和 $b$ 称为
        - **可比的**： $a \preceq b$ 或 $b \preceq a$
        - **不可比的**：既没有 $a \preceq b$，也没有 $b \preceq a$
    - **全序集/线序集**：$(S,\preceq)$ 是偏序集，且 $S$ 中的每对元素都是可比的
        - $\preceq$ 称为**全序或线序**
        - 一个全序集也称为**链**
    - **良序集**：对于偏序集 $(S,\preceq)$，$\preceq$ 是全序，并且 $S$ 的每个非空子集都有一个最小元素
