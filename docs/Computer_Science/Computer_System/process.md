...
- **步骤**
    ...
    - **上下文切换**
        - 父进程 CPU 进入 PCB
        - 子进程 PCB 进入 CPU
    - 子进程**执行**（在上下文切换后，`ret_from_fork` -> `ret_to_user` -> `kernel_exit`（负责恢复 `pt_regs` 中的寄存器））    

!!! abstract "Note"
    1. 可以使用 `getpid()` 和 `getppid()` 获取当前进程的`pid`和`ppid`
    2. **子进程只是父进程的副本，无法直接变成另一个程序，如果你只想运行一个不同的程序，仅靠 `fork` 是不够的**
    3. 当 `fork` 完成后，子进程处于就绪状态，通过上下文切换变为运行状态
...
1. **定义**：当CPU从一个进程切换到另一个进程时，系统必须**保存旧进程的状态**并通过上下文切换**加载新进程的已保存状态**（**发生在内核态**）
...
- 等待执行的进程寄存器状态存在**PCB**中（`cpu_context`结构中）
...
6. **返回**：发生上下文切换后，运行新进程；新进程就绪后，再次调用`cpu_switch_to`，从`schedule()`中返回到原进程

7. **比较内核态和用户态**：上下文保存

- **内核态**
    - **何时**：在 `context_switch` 中（在 `cpu_switch_to` 中） 
    - **何地**：在 `PCB` 中（在 `cpu_context` 结构体中） 
- **用户态**
    - **何时**：在内核入口 `kernel_entry`
    - **何地**：每个线程的**内核栈**中（在 `pt_regs` 结构体中）
