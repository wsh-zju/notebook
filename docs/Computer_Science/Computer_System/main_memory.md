...
- **上下文切换**：每个进程都有自己的页表，切换进程需要**切换页表**，TLB 必须与页表保持一致
    ...
    - 方案 II：用**地址空间标识符** (ASID) 标记 TLB 表项，用以唯一标识一个进程，只刷新对应的表项
...
### 二级页表
!!! warning "**一级页表**"
    1. 如果只有一级的页表，那么**页表所占用的内存**很大：**4 MB**

    - **32 bits 地址为 4 GB**，每个页表 4 KB：一共需要 1 M 的页表项（1 K 个 page）
    - 一个页表项 4 B，占用的内存 1 M $\times$ 4 B = 4 MB

    2. 页表一定是**物理连续的**（physically contiguous），否则还需要另一个页表进行映射
