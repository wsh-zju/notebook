...
- 是 SJF（最短作业优先）调度的一种形式，可能**存在饥饿现象**
...
- 以 **page** 为增量进行读写（类似扇区）
- 但**无法就地重写**，必须先被擦除，且必须擦除 page 所在的整个 block
- 在磨损前**只能擦除有限次数**
- 寿命通过**每日全盘写入次数** (DWPD) 来衡量
...
- **数据条带化** (Data Striping)：将数据分散在多个磁盘上以**允许并行读取** **e.g.** 从 8 个磁盘读取一个字节的各个位
