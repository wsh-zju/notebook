...
- **传统 Unix**：仍未完全分层
    ...
    ??? info "传统 Unix 结构"
        ![alt text](photo/10-9.png)

- **Linux 系统结构**：**本质是单体系统**，但支持**可加载内核模块** (`LKMs`)，增加了灵活性

    ![](photo/10-10.png){style="width:30%;display: block;margin: 20px auto"}

!!! abstract "模块化"
    1. 许多现代操作系统实现了**可加载内核模块** (LKMs)
    2. **特点**
        - 使用面向对象方法
        - 每个核心组件是分离的
        - 每个组件通过已知接口相互通信
        - 每个都可以根据需要在内核中加载
    3. 与分层类似，但更灵活
    4. **e.g.** `Linux`、`Solaris`等

3. **分层系统**：基于抽象层次

![](photo/10-7.png){style="width:40%;display: block;margin: 20px auto"}

- 操作系统被划分为若干层，每一层构建在较低层之上
- 最底层（0）是硬件；最高层（N）是用户界面
- 通过模块化选择各层，使得每一层**仅使用更低层级**的函数（操作）和服务

4. **微内核**

![](photo/10-6.png){style="width:80%;display: block;margin: 20px auto"}

- **设计思想**：将尽可能多的功能从内核移入用户空间
- **实例**：`Mach`（`Mac OS X` 的 `Darwin` 内核部分基于它）
- **通信方式**：用户模块间通过**消息传递**进行通信
- **缺点**：开销比较大，效率比较低，因为用户空间和内核空间之间需要消息传递
- **优点**：更模块化，容易扩展和移植到新架构，容易开发和维护，更稳定可靠和安全

5. **混合内核**：大多数现代OS并非纯粹模型，而是混合系统，以兼顾**性能、安全、可用性**

#### 常见OS

1. **Linux & Solaris**：主要是**单体内核**（位于内核地址空间），同时兼具**模块化**以支持功能动态加载
2. **Windows**：主要是**单体内核**，同时为不同子系统个性化地使用**微内核**
3. **macOS & iOS**：混合+分层

![](photo/10-8.png){style="width:50%;display: block;margin: 20px auto"}

- **上层**：丰富的用户体验（Aqua UI）和框架（Cocoa）
- **内核层**（Darwin）：混合体，包含 `Mach` 微内核、`BSD Unix` 部分、`I/O` 工具包和动态可加载的内核扩展

4. **Android**：

![alt text](photo/10-11.png){style="width:30%;display: block;margin: 20px auto"}

- **栈**：与 `iOS` 相似
- 基于 `Linux` 内核但是加以修改：
    - 提供进程、内存、设备驱动管理
    - 增加了电源管理
- **运行时环境**：核心库集+ `Dalvik` 虚拟机
- **应用开发**：`Java` + `Android API`
- **库**：`Web` 浏览器 (`webkit`)、数据库 (`SQLite`)、多媒体等框架以及更小的 `libc`


5. **openEuler**：`EulerOS` 是一个基于 `Linux` 的服务器操作系统，主要维护者是华为
...
??? abstract "构建与启动 Linux"
    1. 下载 [Linux 源代码](http://www.kernel.org)
    2. 通过 `make menuconfig` 配置内核
    3. 使用 `make` 编译内核
...
- 当系统**加电初始化**时，执行从一个**固定**的内存位置开始
- 引导加载程序**加载内核**

    !!! info "引导加载程序"
        1. **作用**：用于定位内核，将其加载到内存，并启动它
        2. **常见引导加载程序**：`GRUB`（允许从多个磁盘、版本、内核选项中选择内核）
        3. 引导加载程序通常允许**各种启动状态**，例如单用户模式
