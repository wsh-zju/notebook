...
!!! success "改进"
    为了让指令执行完成的顺序也是顺序的，我们添加了一个 **reorder buffer**：结果先写到 ROB，再按照指令流出的顺序 **commit**，写回寄存器
    
    ![alt text](photo/17-14.png){style="width:40%;display: block;margin: 20px auto"}
...
- **`Commit`**：按照指令进入的顺序更新寄存器

3. **三个关键思想**

- 动态分支预测
- **推测执行**：允许在有跳转结果之前执行后边指令（并能够撤销错误推测序列的影响）
- **动态调度**：允许指令跨越跳转指令乱序执行

!!! abstract "精确异常"
    1. **精确异常**：当异常发生时，处理器的状态必须看起来就像指令是严格按照程序顺序一条一条执行完的一样
    2. 而 ROB 可以解决**之前算法非精确异常的问题**，保证了精确异常
