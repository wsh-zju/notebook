...
2. <mark>逻辑块是**最小的传输单位**，被顺序映射到磁盘的扇区中</mark>
3. <mark>**Positioning time**</mark>(random-access time)：将磁臂移动到所需扇区的时间，包括**寻道时间和旋转延迟**
...
5. **平均 I/O 时间**：平均访问时间 + (待传输数据 / 传输速率) + 控制器开销
    ...
    !!! warning "Warning：主要慢在寻道时间和旋转延迟（不好提升）"
...
7. <mark>**磁盘带宽**：数据传输的速度，即 数据/时间</mark>
...
1. **最短寻道时间优先**：
    ...
    1. 是 SJF（最短作业优先）调度的一种形式，可能**存在<mark>饥饿</mark>现象**
    2. 与 SJF 不同，SSTF **<mark>可能不是最优的</mark>**
...
### SCAN
1. <mark>**elevator algorithm**</mark>
...
2. **实现**：磁头在每个方向上<mark>**仅移动到最后一个请求所在的位置**（不会移动到 disk 的末尾）</mark>
...
1. 如果特性类似磁盘驱动器，则称为<mark class="cyan">**固态硬盘 (SSDs)**</mark>

    !!! quote "Quote"
        其他形式包括 USB 驱动器（优盘、闪存盘）、DRAM 磁盘替代品、主板表面贴装存储以及主内存

2. **优点**：可以比 HDD 更可靠；速度快得多（<mark>没有移动部件，因此**没有寻道时间或旋转延迟**</mark>）
3. **缺点**：单位 MB 价格更贵；寿命可能较短，需要仔细管理；容量较小；总线可能太慢
4. <mark>**调度算法**：FCFS</mark>
5. <mark>**特性**</mark>：
...
6. **NAND 闪存控制器算法**
    ...
    4. <mark>每个单元都有寿命，因此**需要均衡地写入所有单元**</mark>

---
## 磁带
1. 磁带是早期的辅助存储类型，现在**主要用于<mark>备份</mark>**
2. **优点**：容量大；存储在磁带上的数据**相对永久**
3. **缺点**：访问时间慢，尤其是<mark>随机访问</mark>（寻道时间远高于磁盘，随机访问需要卷带/倒带）

---
## 磁盘管理

1. <mark>**物理格式化**：将磁盘划分成扇区，以便控制器进行读/写</mark>
    1. **扇区结构**：包含头部信息、数据以及纠错码 ECC
2. 操作系统在磁盘上**记录其自身的数据结构**，将磁盘分区为若干柱面组，每个组被视为一个**逻辑磁盘**
3. <mark>**逻辑格式化**：分区以在其上**建立文件系统**（文件 I/O 以**簇**为单位完成）</mark>
4. **根分区**包含 OS，Mounted at boot time

    1. 其他分区可以包含其他操作系统、其他文件系统，或者是原始分区
    2. 其他分区可以**自动或手动挂载**

5. **引导块 Boot block**：包含足够的代码以了解如何从文件系统中**加载内核**
...
3. **层级**
    ...
    === "RAID 0"
        1. RAID 0：<mark>数据条带化</mark>分布在多个磁盘上，**没有校验位（<mark>无冗余</mark>）**
        2. 提高性能，但不可靠
    ...
    === "RAID 1"
        1. <mark>在两个磁盘上保存一组数据的完全副本（或**镜像**）</mark>
...
!!! info "RAID 与文件系统"
    1. <mark class="blue">**RAID 只能检测/从磁盘故障中恢复，不能防止或检测数据损坏或其他错误**</mark>
