...
!!! tip "Tips"
    1. `a`为指针，`a[i] ＝ *(a+i)`
    2. `*p++`: `++`和`*`的优先级相同，**从右向左**运算
    3. ⽆论指向什么类型，所有的指针的**⼤⼩都⼀样**，因为都是地址
    4. 但是指向不同类型数据的指针不能直接互相赋值的 ，为了避免⽤错指针
## 数组与指针
### 函数
1. **函数原型/声明**
...
### 字符串函数
!!! abstract "单字符的输入和输出"
    1. `getchar()`: 读取但不存储输入；包括`'\n'`
    2. `putchar(char a)`: 返回写了几个字符，返回`EOF(-1)`表示写失败
...
=== "长度计算函数"
    ...
    !!! tip "Tips"
        `\0`标志字符串的结束，但不是字符串的⼀部，计算字符串长度时不包含末尾的`\0`.
    
    !!! abstract "sizeof与strlen"
...
??? abstract "字符串的修改"
    1. 当指针**直接指向字符串常量**时，字符串常量通常存储在**只读数据段**，**不能**通过指针去修改字符串常量的内容（否则会导致运行时错误，如**段错误**）

    e.g.  `char* s = "Hello, world!";`

    `s`实际上是`const char* s`，不能对`s`所指的字符串做写⼊和修改（即不能之后写`s[0]='H';`）

    2. 对于**指向动态分配内存**的指针字符串，其存储的字符串在动态分配的**堆空间**中，**可以**根据实际情况进行修改（只要操作合法且在分配的内存范围内）
    3. 如果需要修改字符串，应该⽤**数组**：`char s[] = "Hello, world!";`