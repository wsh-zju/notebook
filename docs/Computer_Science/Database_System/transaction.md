...
---
!!! success "总结"
    | 性质 / 协议          | 冲突可串行化 | 可恢复 | 无级联 |  严格 | 无死锁 |
    | ---------------- | :-----: | :--: | :--: | :--: | :--: |
    | 冲突可串行化调度         |      是 | 不一定 | 不一定 | 不一定 | 不一定 |
    | 可恢复调度            |    不一定 |   是 | 不一定 | 不一定 | 不一定 |
    | 无级联调度            |    不一定 |   是 |   是 | 不一定 | 不一定 |
    | 严格调度             |    不一定 |   是 |   是 |   是 | 不一定 |
    | 2PL              |      是 | 不一定 | 不一定 | 不一定 | 不一定 |
    | Strict 2PL       |      是 |   是 |   是 |   是 | 不一定 |
    | Rigorous 2PL     |      是 |   是 |   是 |   是 | 不一定 |
    | Conservative 2PL |      是 | 不一定 | 不一定 | 不一定 |   是 |
    | Tree Protocol    |      是 | 不一定 | 不一定 | 不一定 |   是 |
...
1. **事务**：程序执行的一个单元，它访问并可能更新多个数据项
    1. 通常，<mark>一个事务**由若干 SQL 语句组成**，并以 **commit 或 rollback** 结束</mark>
    2. 事务必须看到一致的数据库
...
4. **Aborted 中止状态**：事务<mark>回滚</mark>后，数据库恢复到事务**开始前**状态，中止后有两种选择：
...
2. <mark>**影子数据库方案**</mark>：简单
    1. 所有更新都在**新创建的数据库副本**上完成
    2. <mark>原始副本（即影子副本）不被修改，由 **`db_pointer`** 指向</mark>
    3. **如果中止**：只需删除新副本
    4. **如果提交**：
        1. 将新副本在内存中的所有页写入磁盘
...
2. <mark>如果一个可能并发的调度等价于某个串行调度，则**该调度是可串行化的**</mark>
...
4. **冲突可串行化**
    ...
    2. <mark>如果调度 S 与某个**串行调度冲突等价**，则 S 是**冲突可串行化的**</mark>
...
!!! info "视图可串行化"
    ...
    1. **视图可串行化**：首读一致、写读一致、末写一致
    2. 每个冲突可串行化调度一定是视图可串行化，但视图可串行化不一定冲突可串行化
    3. <mark>视图可串行化但不是冲突可串行化的调度，通常含有 **blind write**</mark>
    4. Blind write 盲写：事务没有先读数据项，就直接写该数据项
...
3. **无级联调度**：对于任意事务 Ti 和 Tj，如果 Tj 读取了 Ti 之前写入的数据项，则 <mark>**Ti 的 commit 操作必须出现在 Tj 的 read 操作之前**</mark>
    1. 每个无级联调度也都是**可恢复的**
    2. <mark>**不会产生级联回滚**：已经提交的数据不会再被回滚</mark>
    3. **最好将调度限制为无级联调度**
...
2. <mark>**一个调度是冲突可串行化的，当且仅当它的优先图无环**</mark>
    1. 存在 $O(n²)$ 时间的**环检测算法**，其中 $n$ 是图中顶点数
    2. **更好的算法复杂度**为 $O(n+e)$，其中 $e$ 是边数
3. <mark>如果优先图无环，可以通过**拓扑排序得到串行化顺序**</mark>
