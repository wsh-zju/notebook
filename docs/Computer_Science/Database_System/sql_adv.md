...
2. <mark>`check` 子句允许**限制域**</mark>：`CONSTRAINT value-test` 子句是可选的，有助于指示哪个约束被违反了
...
2. 假设存在关系 r(A, B, C), s(B, D)，我们说 r 中的属性 B 是来自关系 r 的**外键**，r 称为**参照关系**，s 称为**被参照关系**
    ...
    !!! warning "Warning"
        ...
        2. <mark class="orange">参照完整性**仅在事务结束**时检查！</mark>
...
3. **外键**：
    ...
    - <mark>一般引用被参照关系中的主键（默认）</mark>
    - 可以显式指定被引用表中的引用列，但**这些列必须被声明为主键/候选键（UNIQUE）**
...
4. **相关操作**：
    1. **插入**：向参照关系中插入元组，必须确保该外码的值在参照关系中存在
    2. **删除**：删除被参照关系中的元组，需要查看参照关系中引用该元组的集合，如果这个集合不为空，那么：
        - 要么拒绝删除命令作为错误
        - 要么必须删除参照关系中引用该元组的元组（**级联删除是可能的**）
    3. **更新**：更新外键的值，需要进行与插入类似的检查
...
### 断言
1. **断言**是一个谓词，表达了我们希望数据库**始终满足**的条件，<mark>用于**多个关系**上的复杂检查条件</mark>
...
2. 设计触发器机制时，<mark>必须指定触发器执行的**条件**以及触发器执行时要采取的**动作**</mark>

    ```sql
    CREATE TRIGGER <trigger-name>
    AFTER / BEFORE <event> ON <table-name>
...
!!! example "Example"
    ...
    BEGIN
        INSERT INTO log
        VALUES ('update', o.balance, n.balance);
    END;
    ```
...
4. <mark>授权从一个用户到另一个用户的传递可以用**授权图**表示</mark>（节点是用户，根是数据库管理员）
...
3. <mark>**基本用法**：</mark> `EXEC SQL <embedded SQL statement> END_EXEC`
...
4. **单行查询**
    ...
    - `:V_an, :bn, :bal` 是<mark class="cyan">宿主变量</mark>，可**在宿主语言程序中赋值**，从而将值带入 SQL
    - 宿主变量在宿主语言中使用时**不加 `:` 号**
...
6. <mark>**多行查询**</mark>
    ...
    - **Step 1**：<mark>用 SQL 指定查询并为其**声明一个游标**</mark>
    ...
    - **Step 3**：`FETCH` 将查询结果中一个元组的值**放入宿主语言变量**，并移动指针到下一行（<mark>重复调用以获得后续元组</mark>）
...
1. <mark>**动态 SQL 的本质**是将 SQL 语句视为一个字符串，在程序运行过程中**动态构建**</mark>，并通过 `PREPARE` 和 `EXECUTE` 两个阶段交给数据库处理

2. 上述程序包含一个 **`?`**，这是一个**占位符**，用于替换在执行 SQL 程序时通过 **`USING`** 变量提供的值

## ODBC

1. **ODBC 开放数据库互连**：应用程序与数据库服务器通信的<mark>标准 API</mark>
2. <mark>**特点**</mark>
