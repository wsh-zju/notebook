...
!!! warning "警告"
    `SELECT` 子句中**聚合函数之外的属性**必须出现在 `GROUP BY` 列表中
...
3. **测试空关系**：如果作为参数的子查询非空，则 **`exists`** 结构返回真值 `true`
...
4. **测试重复元组的缺失**：**`unique`** 结构用于测试子查询的结果中是否存在重复元组
...
1. **派生关系**：其元组并不直接物理存储在数据库中，而是根据定义的规则从底层表中提取、组合或计算出来的**虚拟表**
2. 派生关系**最常见的形式**：视图或派生表（派生表必须有自己的别名）
3. **`WITH` 子句**：允许为查询**局部**定义视图，而不是全局定义
...
- **内连接**：只保留满足连接条件的元组
- **左外连接**：保留左侧表的所有行，匹配右侧的表
- **右外连接**：保留右侧表的所有行，匹配左侧的表
- **全外连接**：保留所有行，匹配的空列用 `NULL` 填充
