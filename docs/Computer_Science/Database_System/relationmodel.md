...
- **主码**：
    ...
    - 通常用**下划线**标记
- **外码**：
    - 假设存在关系 r 和 s 满足 r(A, B, C)，s(B, D)，则关系 r 中的属性 B 是引用 s 的外键
    - r 是参照关系，s 是被参照关系
...
![alt text](photo/2-2.png){style="width:40%;display: block;margin: 20px auto"}

4. **并**：合并两个关系，去除重复行

![alt text](photo/2-3.png){style="width:40%;display: block;margin: 20px auto"}

5. **差**：从第一个关系中删除第二个关系中存在的行

![alt text](photo/2-4.png){style="width:40%;display: block;margin: 20px auto"}

6. **笛卡儿积**：生成所有可能的组合

![alt text](photo/2-5.png){style="width:40%;display: block;margin: 20px auto"}

!!! warning "注意"
    当不同表中属性名字相同时，则需要使用**表名.属性名**来区分

7. **改名**：给关系中的属性重新命名 $\rho_{X}(E)$
...
![alt text](photo/2-6.png){style="width:40%;display: block;margin: 20px auto"}
...
![alt text](photo/2-8.png){style="width:40%;display: block;margin: 20px auto"}

4. **除**：从关系中删除满足条件的行

![alt text](photo/2-9.png){style="width:40%;display: block;margin: 20px auto"}
...
!!! success "Summary"
    1. 并、集合差、集合交 —— 为双目、等元运算
    2. 笛卡儿积、自然连接、除 —— 为双目运算
    3. 投影、选择 —— 为单运算对象（即单目运算）
