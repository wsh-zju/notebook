...
3. **锁兼容矩阵**
    ...
    | ---------- | ---------- | ---------- |
    | **S（共享锁）**     | true（<mark>兼容</mark>）   | false（不兼容） |
    ...
    !!! warning "WARNING"
        ...
        所以需要 locking protocol，即加锁和解锁必须遵守规则。

4. **缺点**：
    1. 可能会产生<mark>死锁（不可避免）</mark>
    2. 如果并发控制管理器**设计不当**，也可能发生<mark>**饥饿**</mark>

### <mark class="orange">**两阶段锁 2PL**</mark>
1. **阶段**：
    1. **Growing Phase**：事务可以获得锁，不能释放锁
    2. **Shrinking Phase**：事务可以释放锁，不能**再**获得锁
2. 每个事务都有一个<mark class="cyan">**锁点**（lock point）</mark>：事务获取的最后一个锁的时间点
3. <mark>**优点**：该协议**保证可串行化**</mark>，事务能够按照它们的**锁点（Lock Point）**顺序进行串行化
4. <mark>**缺点**</mark>：
    1. 但是**不能保证避免死锁**
    2. 可能发生**级联回滚**
        1. **strict 2PL**：事务必须一直持有**其所有排他锁**，直到事务提交或中止
        2. **Rigorous 2PL**：事务必须**保持其所有锁**，直到事务提交或中止（<mark>事务可以按照它们的提交顺序进行串行化</mark>）
...
!!! tip "TIPs"
    ...
    3. <mark>冲突可串行化调度不一定能由 2PL 产生；2PL 能产生的一定冲突可串行化</mark>
    4. <mark>判断一个调度是否可能由 2PL 产生</mark>：先看冲突图是否有环，再检查是否存在 unlock 之后又 lock
...
??? info "带锁转换的 2PL"   
    ...
    3. 该协议**保证可串行化**，但它仍然依赖程序员在程序中插入各种加锁指令
...
1. **基于图的协议（Graph-based Protocols）**是两阶段锁协议的另一种替代方案
    ...
    !!! note "NOTE"
        在所有数据项集合 $D={d_1,d_2,\ldots,d_h}$ 上施加一个<mark>**偏序关系** $\rightarrow$</mark>
...
2. **树协议（Tree Protocol）**是一种简单的基于图的协议
    ...
    !!! tip "优点 & 缺点"
        1. **优点**：
            1. <mark>树协议**保证了冲突可串行化**，并且**避免死锁**</mark>
            2. 在树锁协议中，解锁可以比两阶段锁协议更早发生：缩短等待时间，提高并发性
        2. **缺点**：
            1. <mark>协议**不保证可恢复性或无级联回滚**</mark>，需要引入提交依赖以确保可恢复性
            2. 事务可能必须锁定自己不访问的数据项：增加锁开销，延长等待时间，并且可能降低并发性
        3. <mark>在树协议下可能实现的调度，在两阶段锁下未必可行，反之亦然。</mark>

--- 
## \* 基于时间戳的协议
1. 每个事务在进入系统时会**被分配一个时间戳**：若旧事务 Ti 的时间戳为 TS(Ti)，新事务 Tj 会被分配时间戳 TS(Tj)，<mark>**且 TS(Ti) < TS(Tj)**</mark>
2. <mark>**时间戳排序协议**保证任何发生冲突的读、写操作都按照时间戳顺序执行</mark>
...
!!! abstract "Thomas’ Write Rule"
    Thomas 写规则改为：TS(Ti) < WTS(Q)  → 忽略该 write，不回滚

    原因：Ti 写的是过时值，这个写不会影响最终结果
    
    Thomas Write Rule 不保证冲突可串行化，但保证视图可串行化
---
## \* 基于验证的协议
...
2. 每个事务 Ti 有 **3 个时间戳**：
    ...
    4. 为了提高并发性，可串行化顺序**由验证时赋予的时间戳**决定（<mark>因此，TS(Ti) 被赋值为 Validation(Ti)</mark>）
...
2. **数据结构**：
    1. <mark>定义一个数据粒度的层次结构：数据库 → 表 → 页 → 行/记录</mark>
    2. 其中较小粒度嵌套在较大粒度内部，并可用**树形结构**表示（不要与树锁协议混淆）
    3. <mark>当事务显式地锁定树中的某个节点时，它会以相同的锁模式**隐式地锁定**该节点的所有后代节点</mark>
...
### 意向锁
1. <mark>**Intention Locks**：当事务要在某节点加锁时，先在该节点的**所有祖先节点加意向锁**</mark>
2. **意向锁类型**：
...
4. **作用**：
    1. <mark>不用扫描整棵子树，就能判断高层节点能否加 S/X 锁，**只需查祖先节点的意向锁**</mark>
    2. 防止与后代锁冲突，提高效率
5. **事务 Ti 对节点 Q 加锁规则**：
...
4. **基于时间戳的方案**
    1. **Wait-die 方案**（非抢占式）：
        1. 老事务可以等待年轻事务释放锁；<mark>年轻事务不能等待老事务，否则回滚</mark>
    ...
    2. **Wound-Wait 方案**（抢占式）
        1. <mark>老事务不会等待年轻事务；老事务“伤害”年轻事务，强制其回滚</mark>
        2. 年轻事务可以等待老事务
    ...
    !!! success "Success"
        在 wait-die 和 wound-wait 两种方案中，被回滚的事务会**使用其原始时间戳重新启动**。<mark class="green">因此，较老的事务始终优先于较新的事务，从而**避免饥饿**（Starvation）现象。</mark>

5. **基于超时的方案**（Timeout-Based Schemes）：
    1. 事务对锁的等待时间只允许持续一个指定时长
    2. <mark>超时则**事务被回滚**</mark>，因此系统不会无限等待，死锁不可能发生
    3. **优点**：实现简单
    4. **缺点**：<mark>可能发生**饥饿**</mark>；很难确定一个合适的超时时间

### 死锁检测与恢复
1. 死锁可以用**等待图**来描述，<mark>当且仅当等待图中存在**环**时，系统处于死锁状态</mark>
...
4. 回滚（Rollback）：需要决定事务**回滚到什么程度**：
    ...
    2. <mark>**部分回滚**（Partial Rollback）：**只回滚到足以打破死锁的位置，通常更有效**</mark>
