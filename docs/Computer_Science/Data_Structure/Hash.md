...
3. **标识符密度**（Identifier density）= $\frac{n}{T}$

4. **装载密度**（Loading density）$\lambda$ = $\frac{n}{sb}$
...
=== "**线性探测**"
    1. **原理**：当哈希函数计算出的初始位置已被占用时，顺序检查下一个位置，直到找到空槽或遍历整个表
    2. **核心公式**
        - **探测函数**：$f(i)=i$
        - **位置计算**：  
...
=== "**双哈希**"
    1. **原理**：使用两个哈希函数，其中第一个用来计算初始哈希值
    2. **探测函数**：$f(i)=i \times hash_2(x)$
    3. 常用的**第二个哈希函数**：$hash_2(x)=R-(x\%R), 其中R是小于哈希表大小的质数$
