...
!!! info "计算问题 Easy or Hard"
    ...
    2. **最难的问题**：不可判定问题（不存在任何算法能保证对所有输入给出答案 **e.g.** 停机问题）
...
- **P**：在**确定性图灵机**上在多项式时间内**被直接解决**
- **NP**：问题的解可以在**确定性图灵机**上在多项式时间内**被验证**
- **co-NP**：问题的**补问题属于 NP** $\bar{L} \in NP$
...
!!! warning "Notices"
    ...
    3. 已知 $P \subseteq NP$
    4. 某些图论问题（如欧拉回路、最短路径）可以在多项式时间内求解，而与之形式相似的问题（如哈密顿回路、最长路径）却是 **NP-Hard** 问题，目前不存在已知的多项式时间算法


!!! example "Example"
    **Problem**: 哈密顿回路问题

    **Answer**: 给定一条回路，只需要验证该回路是否包含了所有顶点即可，这一验证过程可以在多项式时间内完成，因此**该问题属于 NP**
...
- 一个问题若是**NP完全问题**，那么NP中的任意问题都可以**在多项式时间内归约**到该问题
- 如果有一个**NP完全问题**能够在多项式时间内被解决，那么**所有NP问题都能在多项式时间内解决**
...
!!! info "SAT"
    1. 第一个被证明为**NP完全**的问题：可满足性问题（SAT），它是NP完全研究的起点
