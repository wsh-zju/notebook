...
- **P**：问题本身可以在多项式时间内**被直接解决**
- **NP**：问题的解可以在多项式时间内**被验证**
...
4. **多项式归约定理**：若对任意输入 $α$ 属于问题 $A$，能找到一个多项式时间算法将其转换为问题 $B$ 的输入 $β$，即：

- 转换过程 $R(\alpha) \rightarrow \beta$ 的时间复杂度为 $O(N^{k1})$
- 解 $β$ 的过程时间复杂度为 $O(N^{k2})$
