...
2. **断言 2**：对于 k 路归并，有 $k$ 阶斐波那契数 $F_N^k = F_{N-1}^{(k)} + F_{N-2}^{(k)} + \cdots + F_{N-k}^{(k)}$ 其中 $F_N^{(k)} = 0$，$F_{k-1}^{(k)} = 1$
...
!!! note "解"
    ...
    3. 归并排序输入缓冲区中的两块数据，将数据依次写入输出缓冲区
...
- **假设**：有4个长度分别为2、4、5、15的runs

![](images/11-5.png){style="width:60%;display: block;margin: 20px auto"}