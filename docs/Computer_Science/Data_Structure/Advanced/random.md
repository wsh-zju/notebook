...
int OnlineHiring ( EventType C[], int N, int k){
    ...
    for ( i=1; i<=k; i++) {
        Qi = interview( i );  // 面试前 k 个人，只记录最佳质量，不雇佣
        if ( Qi > BestQ )  
            BestQ = Qi;
    }
    for ( i=k+1; i<=N; i++) {
...
- **思路**：只面试前 $k$ 个人，记录最佳质量，然后从第 $k+1$ 个人开始逐个面试，**如果面试的人质量更好，就雇佣他并立即停止面试**；如果没有找到更好的人，就雇佣最后一个人
...
!!! info "数学分析"
    ...
    5. **概率最大化的 $k$ 值**：对 $\frac{k}{N} \ln (\frac{N}{k})$ 求导取0，得 $k=\frac{N}{e}$，则最大值为 $\frac{1}{e}$
