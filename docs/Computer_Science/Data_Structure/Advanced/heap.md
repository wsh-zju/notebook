...
2. **空路径长度 $Npl(X)$**：任意节点 $X$ 的空路径长度 $Npl(X)$ 是 $X$ 到**不具有两个子节点的节点**的最短路径长度

- 定义 $Npl(NULL) = –1$
- **递推**：$Npl(X) = min \{ Npl(C) + 1 | C 是 X 的所有子节点 \}$
...
!!! tip "Tips"
    ...
    3. **斜堆的缺点**：理论分析复杂，平均右路径长度的期望仍是未完全解决的开放问题
...
!!! abstract "Abstract"
    唯一可以更改重/轻状态的节点是最初处于正确合并路径上的节点!

    轻节点可能变成重节点，重节点一定变成轻节点
5. **摊还分析**
