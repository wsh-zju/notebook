...
## 伸展树 Splay
1. **目标**：从一棵空树开始，连续进行 $M$ 次操作，总耗时不会超过 $O(M \log N)$ （并不意味每一步都是 $O(\log N)$）
...
!!! tip "Notices"
    伸展操作不仅会把被访问的节点移动到根节点，还会使**访问路径上大多数节点的深度大致减半**
...
4. **定理**：一棵有 $N$ 个内部结点的红黑树的高度至多为 $2\log(N+1)$，至少为 $\log(N+1)$
...
!!! tip "Notices"
    ...
    如果节点是红色的或者只有左子树/右子树，则破坏了**黑路同**的性质。
