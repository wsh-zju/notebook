...
- **思路**：按 $x$ 坐标排序并进行**划分**，分成左半部分、右半部分以及**跨越分割线**的三部分解来**递归求解**
- **跨越分割线的解法**（合并阶段）
    ...
    - 如果带状区域内的点数为 \( O(\sqrt{N}) \)，使用**遍历**，时间复杂度为 \( O(N) \)
    ...
    - 对于任意点 \( p_i \) ，**最多只需要考虑7个点**和其之间的距离（在一个 $\delta \times 2\delta$ 的矩形中，最多只能放8个点，且任意两点距离 $≥ \delta$）
    - 从而**时间复杂度** \( f(N) = O(N) \)
