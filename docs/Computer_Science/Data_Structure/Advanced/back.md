...
# 回溯算法
1. **思路**：通过系统地生成和检查部分候选解，并在确定该部分解不可能导向最终正确解时，提前放弃该分支（即**剪枝**），从而减少需要检查的候选解数量
2. **代码模版**
...
bool Reconstruct(DistType X[], DistSet D, int N, int left, int right){ 
    /* 前提: X[1]...X[left-1] 和 X[right+1]...X[N] 已经确定 */
    bool Found = false;      //返回值：是否找到完整的解
    if (Is_Empty(D))
        return true;
    D_max = Find_Max(D);     // 找出当前最大距离
    /* 选项1: 将最大距离放在右侧 X[right] = D_max */
    // 检查约束条件：|D_max-X[i]| 是否都在距离集合 D 中
    OK = Check(D_max, N, left, right); 
    if (OK) {                // 添加 X[right] 并更新距离集合 D
        X[right] = D_max;
        // 更新距离集合 D
        for (i=1;i<left;i++) Delete(|X[right]-X[i]|,D);      // 左侧
        for (i=right+1;i<=N;i++) Delete(|X[right]-X[i]|,D);  // 右侧
        // 递归处理剩余部分
...
## 关键策略
!!! tip "Tips"
    在回溯中，优先选择可选**分支最少/约束最强**的变量或集合，能更早发现冲突，从而大幅减少回溯节点数（剪枝）

1. **最小最大策略**

- 通过递归地模拟双方的最佳走法，最终选择对己方最有利的路径
- e.g. **井字棋**（Tic-tac-toe）：人类玩家试图最小化局面的价值，而计算机则试图最大化它
- 为了量化一个局面的“好坏”，需要一个**评估函数** \( f(P) \)
  
    \[ f(P) = W_{\text{Computer}} - W_{\text{Human}} \]

- 根据评估函数，人类玩家选择评估函数值**最小**的解法，计算机则选择评估函数值**最大**的解法
  
    !!! tip "Tips"
        ![](images/4-1.png){style="width:40%;display: block;margin: 20px auto"}
...
!!! success "优化"
    当两种剪枝算法结合使用时，可以将搜索规模限制在仅 $O(\sqrt{N})$ 个节点，其中 $N$ 是完整博弈树的规模
