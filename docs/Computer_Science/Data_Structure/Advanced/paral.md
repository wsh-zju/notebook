...
2. **最坏情况运行时间**：\( T(n) \) 
3. **处理器数量**： \( P(n) = W(n)/T(n) \) 
4. 	$W(n)/p$ 时间表示使用 $p$ 个处理器时计算所需的时间，其中 $p \leq W(n)/T(n)$ （否则，任务将不能有效分配到处理器上，可能会导致过多的空闲时间）
5. 结合**并行计算和串行计算**的时间： \( W(n)/p + T(n) \)
...
```c
// pardo 是伪代码中用来表示并行执行的关键字
for P_i, 1 ≤ i ≤ n pardo      // 告诉每一个处理器需要做的工作（并不是逐层计算）   
    B(0, i) := A(i)           // 初始化第一层
    for h = 1 to log n do         // 层序遍历，每一层并行
        if i ≤ n/2^h
...
!!! warning "缺点"
    1. **隐藏了处理器数量影响**：公式假设有恰好 $n$ 个处理器，但未说明如果处理器**更少**时如何运行
...
for P_i, 1 ≤ i ≤ n pardo
    B(0, i) := A(i)
for h = 1 to log n
    for P_i, 1 ≤ i ≤ n/2^h pardo    // 在每一层上，所有处理器并行（逐层计算）   
        B(h, i) := B(h-1, 2i-1) + B(h-1, 2i)
for i = 1 pardo
    output B(log n, 1)
```
...
- **WD 表示充分性定理**：任何 WD 模式下的算法，都可以在 **\( O(W(n)/P(n) + T(n)) \)** 时间内，由任意 **\( P(n) \)** 个处理器实现，且使用与 WD 表示中相同的**并发写**约定
...
![alt text](images/10-5.png){style="width:60%;display: block;margin: 20px auto"}
...
- **实际排名**
    - **关键**：$A[i]$ 在 $B$ 中的排名一定在它的**左右路标排名**之间  
...
// 初始化标记数组
for P_i, 1 ≤ i ≤ n pardo  
    B(i) := 0  // B(i)=0 表示 A(i) 可能是最大值，1 表示肯定不是最大值
...
for i and j, 1 ≤ i, j ≤ n pardo  
    if ( (A(i) < A(j)) || ((A(i) = A(j)) && (i < j)) )  
        ...
        B(j) = 1  
    // 注意：这里存在并发写入冲突！多个处理器可能同时写入 B(i) 或 B(j)
...
- 使用**并行比较算法**将每个子问题**合并**得到最终最大值 $A_{max} \sim T=O(1),W=O(\sqrt{n}^2)=O(n)$
- **建立递归关系**
...
- 使用**双对数方法 1**将子问题**合并** $\sim T=O(\log \log (n/h)),W=O((n/h)\log \log (n/h))$
- **性能**
...
!!! info "示意图"
    ![alt text](images/10-6.png){style="width:60%;display: block;margin: 20px auto"}

- **步骤**：
    - **随机采样** $n^{\frac{7}{8}}$ 个元素 $\sim T=O(1)\quad W=O(n^{\frac{7}{8}})$
        - 将采样得到的元素按照每块 $n^{\frac{1}{8}}$ 划分为 $n^{\frac{3}{4}}$ 个块
        - 利用**并行比较算法**找到每一块的最大值 $\sim T=O(1)\quad W=O(n)$
        - 继续将**得到的最大值**划分为每块 $n^{\frac{1}{4}}$ 个元素，共 $n^{\frac{1}{2}}$ 块，找到每一块的最大值 $\sim T=O(1)\quad W=O(n)$
        - 对得到的 $n^{\frac{1}{2}}$ 个最大值使用**并行比较算法**得到最终解 $M$ $\sim T=O(1)\quad W=O(n)$
    - **迭代**
        -  **淘汰**小于等于 $M$ 的元素
        -  使用上述**随机采样**方法不断重复
- **定理**：
    - 以极高概率，该算法**在 \( O(1) \) 时间和 \( O(n) \) 工作量内**找到最大值（在任意 CRCW PRAM 上）
    - **未能找到最大值的概率**为 \( O(1/n^c) \)，其中 \( c \) 为某个正常数