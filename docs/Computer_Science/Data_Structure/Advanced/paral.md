...
- **步骤**：
    - **随机采样** $n^{\frac{7}{8}}$ 个元素 $T=O(1)\quad W=O(n^{\frac{7}{8}})$
        - 将采样得到的元素按照每块 $n^{\frac{1}{8}}$ 划分为 $n^{\frac{3}{4}}$ 个块
        - 利用**并行比较算法**找到每一块的最大值 $T=O(1)\quad W=O(n)$
        - 继续将**得到的最大值**划分为每块 $n^{\frac{1}{4}}$ 个元素，共 $n^{\frac{1}{2}}$ 块，找到每一块的最大值 $T=O(1)\quad W=O(n)$
        - 对得到的 $n^{\frac{1}{2}}$ 个最大值使用**并行比较算法**得到最终解 $M$ $T=O(1)\quad W=O(n)$
    - **迭代**
        -  **淘汰**小于等于 $M$ 的元素
        -  使用上述**随机采样**方法不断重复
- **定理**：
