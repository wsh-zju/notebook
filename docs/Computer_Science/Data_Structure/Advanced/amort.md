...
2. **摊还时间界**（amortized time bound）：连续操作总时间的上界

- 最坏情况界 $\geq$ 摊还界 $\geq$ 平均情况界 

---
## 聚合分析
对于任意 $n$，一个包含 $n$ 次操作的序列总共需要的**最坏情况时间**为 $T(n)$

因此，在最坏情况下，每次操作的平均成本（即摊还成本）为 $T(n)/n$
...
!!! abstract "Notices"
    ...
    并且**单调递增的函数**不能用来做势能函数，会导致实际成本与摊还成本之间的差距不断加大
